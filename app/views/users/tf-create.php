...
<body class="bg-gradient-to-br from-blue-900 to-blue-700 min-h-screen">
    ...
    <main class="flex-grow container mx-auto px-4 py-8">
      <div class="max-w-3xl mx-auto bg-white/10 backdrop-filter backdrop-blur-lg rounded-xl shadow-xl overflow-hidden">
        <div class="p-8">
          ...
          <!-- True/False Question Creation Form -->
          <form id="trueFalseForm" action="/quiz/api/save-tf-question" method="POST" class="space-y-6">
            ...
            <div>
              <label class="block text-white text-lg font-medium mb-2">Answer</label>
              <div class="flex space-x-4">
                <button
                  type="button"
                  class="answer-btn w-full bg-green-500 text-white font-bold py-3 px-6 rounded-lg transition duration-300 ease-in-out hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-white"
                  data-answer="True">
                  True
                </button>
                <button
                  type="button"
                  class="answer-btn w-full bg-red-500 text-white font-bold py-3 px-6 rounded-lg transition duration-300 ease-in-out hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-white"
                  data-answer="False">
                  False
                </button>
              </div>
                  ...
                  <div class="p-4">
                    <div class="flex flex-col">
                      ...
                      <div class="flex justify-between items-center mt-4">
                        <span class="text-sm text-gray-300">True/False</span>
                        <div class="space-x-2">
                          <button
  ...
  <div id="deleteModal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-60 backdrop-blur-sm hidden" aria-modal="true" role="dialog">
    <div class="relative bg-white/10 backdrop-filter backdrop-blur-lg rounded-xl shadow-xl w-96">
      ...
      <!-- Form -->
      <form id="deleteForm" action="/quiz/api/delete-tf-question" method="POST" class="px-6 pb-4">
  ...
  <div id="editModal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-60 backdrop-blur-sm hidden" aria-modal="true" role="dialog">
    <div class="relative bg-white/10 backdrop-filter backdrop-blur-lg rounded-xl shadow-xl w-96">
      ...
      <div class="p-6">
        <form id="editForm" action="/quiz/api/update-tf-question" method="POST" class="space-y-6">
          <input type="hidden" id="editQuestionId" name="question_id" value="">
          <input type="hidden" name="quiz_id" value="<?= htmlspecialchars($quiz['quiz_id']) ?>">
          <input type="hidden" name="answerMode" value="<?= htmlspecialchars($quiz['quizType']) ?>">
          ...
          <div>
            ...
            <!-- Hidden input to hold the selected answer -->
            <input type="hidden" id="editAnswer" name="answer" required />
          </div>
  ...
  <script>
    $(document).ready(function() {
      // Handle Answer Button Click in True/False Question Creation Form
      $("#trueFalseForm .answer-btn").on("click", function() {
        const answer = $(this).data("answer");
        $("#answer").val(answer);
        $("#trueFalseForm .answer-btn").removeClass("bg-opacity-50");
      ...
      $(".open-edit-modal").on("click", function() {
        ...
        $("#editForm .answer-btn").removeClass("bg-opacity-50");

        // Highlight the button based on the correctAnswer
        if (correctAnswer === "True") {
          $('#editForm [data-answer="True"]').addClass("bg-opacity-50");
        } else if (correctAnswer === "False") {
          $('#editForm [data-answer="False"]').addClass("bg-opacity-50");
        }
      ...
      // Handle Answer Button Click in Edit Modal
      $("#editForm .answer-btn").on("click", function() {
        const answer = $(this).data("answer");
